...
patch -p1 < $HOME/embeddedgo/patch/go1.18.1
cd src
./make.bash
```
Now you have the ready to use go1.18.1 with added support for linux/thumb, noos/thumb, noos/riscv64 GOOS/GOARCH pairs. You can run tests for your native architecture to ensure that nothing was broken:

```
./run.bash --no-rebuild
```

You can always revert all changes and return back to the clean go1.18.1 using the following commands:
