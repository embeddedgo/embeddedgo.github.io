...
patch -p1 < $HOME/embeddedgo/patch/go1.15.3-2
cd src
./make.bash
```
Now you have the ready to use go1.15.3 with added support for linux/thumb, noos/thumb, noos/riscv64 GOOS/GOARCH pairs. You can run tests for your native architecture to ensure that nothing was broken:

```
./run.bash --no-rebuild
```

You can always revert all changes and return back to the clean go1.15.3 using the following commands:
